...
const Checkout = () => {
  ...
  var store_id = localStorage.getItem("user")
    ? JSON.parse(window.localStorage.getItem("user")).store_id
    : "default_store_id";

  const [dataBank, setDataBank] = useState([]);
  const [paymentMethod, setPaymentMethod] = useState("KIP");
  ...
  const getConvertedAmount = (currency) => {
    const shippingFee = totalPrice < 20 ? 3 : 0; // ค่าจัดส่ง $5 ถ้าราคาน้อยกว่า $20
    return (totalPrice + shippingFee) * exchangeRates[currency]; // แก้ไขให้ไม่ใช้ toFixed(2) เพื่อแสดงเศษทศนิยม
  };
  ...
  const handlePayment = async () => {
    ...
    const templateParams = {
      user_name: district || user_id,
      message: "",
    };

    await emailjs.send("service_a5i5bbi", "humascotl_kgxxecq", templateParams, {
      publicKey: "ztlmTMMWL5WnRAe8K",
    });
    ...
    <>
      <div className="checkout-container">
        ...
        <div className="order-details">
          ...
          <>
            <label htmlFor="account_name">주문자 이름으로 송금해주세요.</label>
            <input
              type="text"
              id="account_name"
              className="address-input"
            ...
            <div className="boxaccount_number_p">
              <div className="containerQRCode">
                <div className="box_qrcode_imag">
                  <div className="box_image_txt">계정 QR코드 이미지 KIP:</div>

                  <img src={QR_Code} alt="" />
                </div>
                <div className="box_qrcode_imag">
                  {/* <div className="box_image_txt">
                        계정 QR코드 이미지 KRW:
                      </div> */}
                  {/* <img src={QR_Code} alt="" /> */}
                </div>
          ...
          <div className="delivery-note">20불 미만은 배송비가추가됩니다</div>
          {totalPrice < 20 && (
          ...
          <div className="total-price">
            <div className="getConvertedAmount">
              총 가격: <div className="ConvertedAmount">USD</div>{" "}
            </div>
            <div className="ConvertedAmount">${getConvertedAmount("USD")}</div>
          </div>

          <div className="total-price">
            <div className="getConvertedAmount">
              총 가격: <div className="ConvertedAmount">KRW</div>{" "}
            </div>
            <div className="ConvertedAmount">
              {paymentMethod === "KIP" &&
                Math.round(
                  getConvertedAmount("KRW") +
                    (getConvertedAmount("KRW") % 1 >= 0.6 ? 0 : 0)
                )
                  .toLocaleString()
                  .replace(/\.\d{1,2}$/, "")}
            </div>
          ...
          <div className="total-price">
            <div className="getConvertedAmount">
              총 가격: <div className="ConvertedAmount">KIP</div>{" "}
            </div>
            <div className="ConvertedAmount">
              {paymentMethod === "KIP" &&
                Math.round(
                  getConvertedAmount("KIP") +
                    (getConvertedAmount("KIP") % 1 >= 0.6 ? 0 : 0)
                )
                  .toLocaleString()
                  .replace(/\.\d{1,2}$/, "")}
            </div>
